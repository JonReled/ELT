...
function Stats() {
    ...
    for (let i = 0; i < Object.keys(retrieveUserPR()).length; i++) {
      ...
      newTableRows.push(
        <TableRow
          ...
          standard={calculateUserExerciseLevel(exercise[0], [
            exercise[1].estimated1RM,
            exercise[1].tested1RM,
          ])}
        />
      );
    }
    ...
    for (let i = 0; i < LogDatabase.length; i++) {
      ...
      for (let j = 0; j < Object.keys(Log).length; j++) {
        ...
        if (!["Bench", "Squat", "Deadlift"].includes(exercise.exerciseName))
          break;
        const currentEstimatedPR = retrieveUserPR()[exercise.exerciseName]
          .estimated1RM;
        const currentTestedPR = retrieveUserPR()[exercise.exerciseName]
          .tested1RM;

        if (
          kemmlerEquation(exercise.Weight, exercise.Reps) > currentEstimatedPR
        ) {
          const estimated1RM = Math.floor(
            kemmlerEquation(exercise.Weight, exercise.Reps)
          );
      ...
      updateUserPR(
        "Total",
        retrieveUserPR().Bench.estimated1RM +
          retrieveUserPR().Squat.estimated1RM +
          retrieveUserPR().Deadlift.estimated1RM,
        "estimated1RM"
      );
      updateUserPR(
        "Total",
        retrieveUserPR().Bench.tested1RM +
          retrieveUserPR().Squat.tested1RM +
          retrieveUserPR().Deadlift.tested1RM,
        "tested1RM"
      );
    }
  }
  ...
  function calculateUserExerciseLevel(exerciseName, user1RM) {
    ...
    for (let j = 0; j < user1RM.length; j++) {
      if (
        user1RM[j] <
        calculateUserWorldRecord(75, "male", exerciseName) *
          Object.values(strengthLevels)[0][0]
      ) {
        levelArray.push([
          Object.keys(strengthLevels)[0],
          Object.values(strengthLevels)[0][1],
        ]);
      }
      ...
      for (let i = Object.keys(strengthLevels).length - 1; i > 0; i--) {
        if (
          user1RM[j] >=
          calculateUserWorldRecord(75, "male", exerciseName) *
            Object.values(strengthLevels)[i][0]
        ) {
          levelArray.push([
            Object.keys(strengthLevels)[i],
            Object.values(strengthLevels)[i][1],
          ]);
          break;
        }
      }
...
function LogTable(props) {
  ...
  function changeLevel() {
    standardType === "Estimated Level"
      ? setStandardType("Tested Level")
      : setStandardType("Estimated Level");
  }
  ...
  return (
    <Table celled padded style={{ maxWidth: "720px" }}>
      ...
      <Table.Body>{props.tableRows}</Table.Body>
    </Table>
  );
}
  ...
  return (
    <Table.Row>
      ...
      <Table.Cell textAlign="center">
        {standardType === "Estimated Level" ? (
          <h1 style={{ color: props.standard[0][1] }}>
            {props.standard[0][0]}
          </h1>
        ) : (
          <h1 style={{ color: props.standard[1][1] }}>
            {props.standard[1][0]}
          </h1>
        )}
      </Table.Cell>
  ...
  if (gender === "male") {
    ...
    for (let i = 0; i < weightClasses.length - 1; i++) {
      if (bw < weightClasses[i]) {
        return Math.round(
          (Object.values(strengthStandards.male)[i][0] * bw +
            Object.values(strengthStandards.male)[i][1]) *
            ratiosToTotal[exerciseName]
        );
      }
    }
  }
